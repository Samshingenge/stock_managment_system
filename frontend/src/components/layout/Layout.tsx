...
function Header({ onMenuClick, title = 'Stock Management System' }: HeaderProps) {
  ...
  const handleExcelExport = async () => {
    try {
      ...
      const exportButton = document.querySelector('[title="Export to Excel"]') as HTMLButtonElement;
      if (exportButton) {
        exportButton.disabled = true;
        exportButton.innerHTML = '<svg class="h-4 w-4 mr-2 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>Exporting...';
      }

      // Fetch current data from all services
      const [products, transactions, suppliers, dashboardStats] = await Promise.all([
        productsService.getProducts({ per_page: 1000 }).then(response => response.items || []).catch(() => []),
        transactionsService.getTransactions({ per_page: 1000 }).then(response => response.items || []).catch(() => []),
        suppliersService.getSuppliers({ per_page: 1000 }).then(response => response.items || []).catch(() => []),
      ...
      if (exportButton) {
        ...
        exportButton.innerHTML = '<svg class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>Export to Excel';
      }
    }
  };
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              ...
              className="inline-flex items-center px-3 py-2 rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-colors duration-200"
